<?php
if($_POST['text1'] != '') {
  $json = file_get_contents('../data/content_2017.json');
    ...
    if(isset($_POST['text2'])){
      ...
      for ($i=0; $i<count($_FILES['userfile']['name']); $i++) {
        ...
        if (/*FileExtensionGetAllowUpload($file_ext) && */ is_uploaded_file($_FILES["userfile"]["tmp_name"][$i])) {
          if(move_uploaded_file($_FILES["userfile"]["tmp_name"][$i], "../img/2017/".$newfilename)) {
            $json = file_get_contents('../data/content_2017.json');
            $records = json_decode($json, true);
            
            $records[] = [
                'title' => $_POST['text1'],
                'image' => "img/2017/{$newfilename}",
                'category' => $_POST['text2']
            ];
            
            $out_json = json_encode($records, JSON_UNESCAPED_UNICODE);
            
            file_put_contents("../data/content_2017.json", $out_json, LOCK_EX);
